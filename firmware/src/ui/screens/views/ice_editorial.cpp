...
static lv_obj_t *s_eyebrow, *s_price, *s_contract, *s_change, *s_status;
...
static void build(lv_obj_t* page) {
  ...
  lv_label_set_text(s_eyebrow, "BEACON / D4 RIN");
  ...
  lv_obj_align(s_price, LV_ALIGN_TOP_LEFT, SAFE_INSET, SAFE_INSET + 34);
...
static void update(void) {
  ...
  for (int i = 0; i < ICE_ED_ROWS; i++) {
    ...
    ice_fmt_price(buf, sizeof(buf), r.c[i].last);        lv_label_set_text(s_row_val[i], buf);
