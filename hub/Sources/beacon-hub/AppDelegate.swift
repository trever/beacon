...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func startTickerEditor() {
        ...
        // The popover still offers "Tickers…" (HubPanel). Its old target, TickerEditorWindowController, is
        // gone -- the editor is the Device tab's ticker section now -- so route the button there instead of
        // leaving it inert. The tab is @AppStorage-backed, so selecting it before show() is what lands us
        // on the right one.
        menubar.onOpenTickerEditor = { [weak self] in
            SettingsTabPersistence.save(.device)
